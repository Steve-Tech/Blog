...
Just copy the link for `rootfs.tar.xz` and paste it in the 'Download from URL' dialog that's in proxmox's storage then 'CT Templates'.
...
Click on a distro, click on a version under arm64 & default, copy the link for `rootfs.tar.xz` and paste it in the 'Download from URL' dialog that's in proxmox's storage then 'CT Templates'.
